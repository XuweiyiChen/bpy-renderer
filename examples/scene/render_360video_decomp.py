...
from bpyrenderer.importer.custom_loaders import modify_obj_vertex_color
...
# 2. Import models
load_file("../../assets/models/compo_scene_created_by_midi3d.glb")

# Others. smooth objects and normalize scene
scene_manager.smooth()
scene_manager.normalize_scene(1.0)

# Others. modify vertex color
COLORS = [
    (0.90, 0.75, 0.70, 1.0),  # Light Pink
    (0.70, 0.80, 0.90, 1.0),  # Sky Blue
    (0.75, 0.85, 0.70, 1.0),  # Mint Green
    (0.90, 0.85, 0.65, 1.0),  # Apricot Yellow
    (0.85, 0.70, 0.80, 1.0),  # Soft Purple
...
for i, obj in enumerate(scene_manager.scene_meshes):
    modify_obj_vertex_color(obj, COLORS[i % len(COLORS)])
