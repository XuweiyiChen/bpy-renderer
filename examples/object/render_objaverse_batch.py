#!/usr/bin/env python3
"""
Batch rendering script for Objaverse GLB files using multiple GPUs.
...
def setup_scene_for_glb(glb_path, env_texture_path):
    """Setup the Blender scene for a specific GLB file"""
    ...
    # 3. Smooth objects and normalize scene
...
def render_glb_multiview(glb_path, output_base_dir, env_texture_path=None, 
    ...
    try:
        ...
        print(f"Process {os.getpid()}: Starting render for {glb_name}")
        
        # Setup scene
        scene_manager = setup_scene_for_glb(glb_path, env_texture_path)
        ...
        print(f"Process {os.getpid()}: Completed render for {glb_name} - {len(frame_info_list)} views")
        ...
        print(f"Process {os.getpid()}: Error rendering {glb_path}: {str(e)}")
        return {
...
def render_glb_batch(glb_files, output_base_dir, env_texture_path=None, 
                    width=512, height=512, num_views=12, n_jobs=8):
    """
    Render a batch of GLB files in parallel using multiple GPUs.
    ...
    Args:
        ...
        n_jobs: Number of parallel jobs (should match number of GPUs)
    ...
    print(f"Starting batch render of {len(glb_files)} GLB files using {n_jobs} parallel jobs")
    ...
    # Parallel rendering using joblib
    results = Parallel(n_jobs=n_jobs, verbose=2)(
        delayed(render_glb_multiview)(
